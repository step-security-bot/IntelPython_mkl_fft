...
This interface is a drop-in replacement for [`numpy.fft`](https://numpy.org/devdocs/reference/routines.fft.html) module and includes all the functions available there:
...
The following example shows how to use this interface for calculating 1D FFT.
...
* Real FFTs: `rfft`, `irfft`, `rfft2`, `irfft2`, `rfftn`, `irfftn`. Note that `overwrite_x` keyword argument in these functions is only supported with its default value.

* Hermitian FFTs: `hfft`, `ihfft`, `hfft2`, `ihfft2`, `hfftn`, `ihfftn`. Note that `overwrite_x` keyword argument in these functions is only supported with its default value.
