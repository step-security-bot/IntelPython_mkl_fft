...
sp_res = scipy.fft.fft2(x)  # Calls default SciPy backend
numpy.allclose(mkl_res, sp_res)
# True
```

To ensure `mkl_fft` is used as the SciPy backend, you can use the `verbose` option.
...
with scipy.fft.set_backend(mkl_backend, only=True):
	mkl_res = scipy.fft.fft2(x, workers=4)
...
The previous examples are only for illustration purposes. In practice, there is no added benefit to defining a backend and calculating FFT, since this functionality is already accessible through the interface, as shown earlier.
